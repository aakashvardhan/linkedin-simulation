...
scenario_A_latency    = [1217, 1119, 1156, 1197]   # ms — JMeter measured, 100 threads 60s
scenario_A_throughput = [74.9, 81.4, 78.8, 76.1]   # req/s — JMeter measured
...
latency_pct = round((scenario_A_latency[0] - scenario_A_latency[1]) / scenario_A_latency[0] * 100, 1)
ax.annotate(f'Redis caching reduces\nlatency by {latency_pct}%',
...
# Throughput improvement B → B+S
improvement_pct = round((scenario_A_throughput[1] - scenario_A_throughput[0]) / scenario_A_throughput[0] * 100, 1)
ax.annotate(f'+{improvement_pct}% throughput\nwith Redis cache',
            xy=(1, scenario_A_throughput[1]),
            xytext=(2.2, scenario_A_throughput[1] * 0.85),
            arrowprops=dict(arrowstyle='->', color='green', lw=1.5),
            fontsize=9, color='green', ha='center')

# Footnote: explain flat profile — indexed DB means cache adds modest but measurable gain
fig.text(0.5, 0.01,
         'Flat profile expected: /jobs/search uses indexed MySQL — baseline already fast. Redis adds ~8% gain on repeated queries.',
         ha='center', fontsize=8, color='#555555',
         bbox=dict(boxstyle='round,pad=0.3', facecolor='#F3F3F3', edgecolor='#AAAAAA'))
...
print('\nKey findings:')
print(f'  Scenario A: Redis reduces latency from {scenario_A_latency[0]:,}ms to {scenario_A_latency[1]}ms ({latency_pct}% improvement)')
print(f'  Scenario A: Throughput increases from {scenario_A_throughput[0]} to {max(scenario_A_throughput):.1f} req/s (+{improvement_pct}%)')
