...
async def explain_ranking(job: dict, candidate: dict, match: dict) -> dict:
    ...
    if not is_llm_configured():
        lines = [
            f"- Match score {score} (semantic {semantic}, overlap ratio {overlap_ratio}).",
            f"- Matched required skills: {', '.join(matched) if matched else '(none)'}",
            f"- Missing required skills: {', '.join(missing) if missing else '(none)'}",
            f"- Extra candidate skills (not required): {', '.join(extras[:8]) if extras else '(none)'}",
            f"- Candidate title: {cand_title or 'n/a'}; years experience: {cand_years if cand_years is not None else 'n/a'}",
        ]
        return {
            "explanation": "\n".join(lines),
            "score": score,
