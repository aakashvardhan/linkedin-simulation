...
def _static_outreach(job: dict, candidate: dict, match: dict, low_match: bool) -> dict:
    job_title = job.get("title", "this role")
    company = job.get("company_name") or job.get("company") or "our company"
    score = float(match.get("score", 0) or 0)
    overlap_list = match.get("skills_overlap", []) or []
    cand_skills = candidate.get("skills", []) or []
    opener = (
        f"Hi — I’m recruiting for {job_title} at {company}. "
        f"Your background ({', '.join((cand_skills or overlap_list)[:5]) or 'your experience'}) "
        f"looks relevant (match score {score:.0%})."
    )
    if low_match:
        body = f"{opener} Would you be open to a quick chat about fit and what you’re looking for next?"
    else:
        body = f"{opener} Would you have 15 minutes this week to connect?"
    return {
        "draft": body,
        "job_title": job_title,
        "company": company,
...
async def generate_outreach(job: dict, candidate: dict, match: dict) -> dict:
    ...
    if not is_llm_configured():
        return _static_outreach(job, candidate, match, low_match)
