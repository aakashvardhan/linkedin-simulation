...
def _job_list_row(job: JobPosting) -> dict:
    """Shape aligned with frontend `mapBackendJobRow` (search / byRecruiter lists)."""
    company_name = job.company.name if job.company else None
    industry = job.company.industry if job.company else None
    try:
        skills = json.loads(job.skills_required or '[]')
    except json.JSONDecodeError:
        skills = []
    wm = (job.work_mode or '').lower()
    return {
        'job_id': job.job_id,
        'id': job.job_id,
        'title': job.title,
        ...
        'remote_policy': job.work_mode,
        'remote': wm in ('remote', 'hybrid'),
        ...
        'applications_count': job.applicants_count,
...
def search_jobs(payload: JobSearchRequest, db: Session = Depends(get_db)):
    ...
    response = success_response(
        {
            'jobs': [_job_list_row(job) for job in jobs],
